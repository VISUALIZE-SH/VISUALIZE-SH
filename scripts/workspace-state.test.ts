...
test('shared evidence context survives deep links; malformed dates and modes are ignored', () => {
  ...
  assert.equal(parseWorkspace('').mode, 'atlas', 'the home page opens Atlas')
  assert.equal(parseWorkspace('').atlasView, 'graph', 'the home page opens the graph')
  assert.equal(parseWorkspace('?mode=unknown&version=%3Cscript%3E').mode, 'atlas')
  assert.equal(parseWorkspace('?mode=unknown&version=%3Cscript%3E').versionId, '')
  assert.equal(parseWorkspace('?mode=news').mode, 'news', 'explicit News links still open News')
